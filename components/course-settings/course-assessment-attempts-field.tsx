...
export function CourseAssessmentAttemptsField({ initialLimit }: Props) {
  ...
  return (
    <fieldset className="flex flex-col gap-3">
      ...
      <p className="text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
        Limit how many times a learner can submit the final lesson assessment.
        Each submit records the score to the LMS (SCORM). Retake clears answers
        and resets the in-player score until they submit again. The last allowed
        submit is the score that remains when attempts are exhausted.
      </p>
      ...
      >
        ...
        <input
          id={countInputId}
          type="number"
          name="assessment_attempts_limit"
          min={1}
          step={1}
          value={count}
          ...
          aria-label="Maximum number of assessment attempts"
        />
        attempt{count === "1" ? "" : "s"}
      </label>
    </fieldset>
